...
{
    public class RightUppercut : Punch
    {
        public RightUppercut(ContentManager Content, float GameWidth, float GameHeight, float GuardY, float ScaleGuard, float ScalePunch)
        {
            rGuard = new Rectangle(0, 0, 450, 600);
            pvGuard = new Vector2(GameWidth / 4, GuardY);
            ovGuard = new Vector2(rGuard.Width / 2, 0);

            rPunch = new Rectangle(0, 0, 526, 1018);
            ...
            tPunch = Content.Load<Texture2D>("right_uppercut");

            base.SetStartingPositions(pvGuard, pvPunch);

            punchName = "Right Uppercut";
        }
    }
}
